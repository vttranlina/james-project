...
//                doc.add(new TextField(BODY_FIELD, textualBody, Field.Store.YES));
    ...
    public Document createFlagsDocument(MailboxMessage message) {
        Document doc = new Document();
        doc.add(new StringField(ID_FIELD, createFlagsIdField(message.getMailboxId(), message.getUid()), Field.Store.YES));
        doc.add(new StringField(MAILBOX_ID_FIELD, message.getMailboxId().serialize(), Field.Store.YES));

        Optional.of(message.getUid().asLong())
            .ifPresent(uidAsLong -> {
        ...
        Flags messageFlags = message.createFlags();
