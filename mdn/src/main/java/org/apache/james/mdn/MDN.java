...
public class MDN {
    ...
    // todo create subclasses of this exception to better detail failure?
    ...
    // todo write tests!
    // todo remove optional usages to fix compilation
    public static MDN parse(Message message) throws MDNParseException {
        if (!message.isMultipart()) throw new MDNParseContentTypeException("MDN Message must be multipart");
    ...
    public static Optional<MDNReport> extractMDNReport(Entity reportEntity) {
        if (!reportEntity.getMimeType().startsWith(DISPOSITION_CONTENT_TYPE)) return Optional.empty();
